...
import { PlusCircle, Send, CheckCircle, Users } from 'lucide-react';
...
export const AdminDashboard = () => {
  ...
  return (
    <div className="py-6 lg:py-12 max-w-3xl">
      <motion.div 
      ...
      >
        ...
        <div className="flex items-center gap-6 mb-12">
          <div className="w-16 h-16 rounded-3xl bg-indigo-500/10 border border-indigo-500/20 flex items-center justify-center text-indigo-400 shadow-2xl">
            {activeTab === 'create' ? <PlusCircle size={32} /> : <Users size={32} />}
          </div>
          <div>
            <h2 className="text-4xl font-black tracking-tighter mb-1">
              {activeTab === 'create' ? 'Architect Dashboard' : 'Human Resource Nexus'}
            </h2>
            <p className="text-slate-500 font-medium">
              {activeTab === 'create' ? 'Curate the knowledge base for future masters' : 'Approve apprentices for the mastery journey'}
            </p>
          </div>
        </div>
        ...
        <div className="flex gap-4 mb-10 border-b border-white/5 pb-4">
          <button 
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold uppercase tracking-widest transition-all ${
              activeTab === 'create' ? 'bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 shadow-lg' : 'text-slate-600 hover:text-slate-400'
            }`}
          >
            <PlusCircle size={14} /> Knowledge Creation
          </button>
          <button 
            onClick={() => setActiveTab('users')}
            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold uppercase tracking-widest transition-all ${
              activeTab === 'users' ? 'bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 shadow-lg' : 'text-slate-600 hover:text-slate-400'
            }`}
          >
            <Users size={14} /> Apprentice Oversight
          </button>
        </div>
